...
const consoleErrors=[];const lateNetworkRequests=[];const lateBlobRequests=[];
try{
  ...
  page.on('request',r=>{if(!armed)return;const u=r.url();if(/^https?:/i.test(u))lateNetworkRequests.push(u);else if(u.startsWith('blob:'))lateBlobRequests.push(u)});
  ...
  const sustained=a.running&&b.running&&c.running&&b.cycles>a.cycles&&c.cycles>b.cycles&&c.dualReady===c.totalIterations&&c.egressRejects===c.cycles&&c.familiesSeen.length===12&&c.history.length<=64&&c.restarts>=1&&unexpected.length===0&&lateNetworkRequests.length===0&&consoleErrors.length===0;
